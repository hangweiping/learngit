...
![img](http://box.kancloud.cn/2015-09-21_55ff97ea87dbc.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff986c28aad.png)
...
上节已经提到，每个函数function都有一个prototype，即原型。这里再加一句话——每个对象都有一个**proto**，可成为隐式原型。

这个**proto**是一个隐藏的属性，javascript不希望开发者用到这个属性值，有的低版本浏览器甚至不支持这个属性值。所以你在Visual Studio 2012这样很高级很智能的编辑器中，都不会有**proto**的智能提示，但是你不用管它，直接写出来就是了。

 ![img](http://box.kancloud.cn/2015-09-21_55ff98a341fe3.png)

 ![img](http://box.kancloud.cn/2015-09-21_55ff98a356e4c.png)

上面截图看来，obj.**proto**和Object.prototype的属性一样！这么巧！

答案就是一样。

obj这个对象本质上是被Object函数创建的，因此obj.**proto**=== Object.prototype。我们可以用一个图来表示。

![img](http://box.kancloud.cn/2015-09-21_55ff98a3713c4.png) 

即，每个对象都有一个**proto**属性，指向创建该对象的函数的prototype。

那么上图中的“Object prototype”也是一个对象，它的**proto**指向哪里？

好问题！

在说明“Object prototype”之前，先说一下自定义函数的prototype。自定义函数的prototype本质上就是和 var obj = {} 是一样的，都是被Object创建，所以它的**proto**指向的就是Object.prototype。

但是Object.prototype确实一个特例——它的**proto**指向的是null，切记切记！

![img](http://box.kancloud.cn/2015-09-21_55ff98a390d85.png)

还有——函数也是一种对象，函数也有**proto**吗？
...
![img](http://box.kancloud.cn/2015-09-21_55ff98a3af854.png)
...
好了，根据上面说的一句话——对象的**proto**指向的是创建它的函数的prototype，就会出现：Object.**proto** === Function.prototype。用一个图来表示。

![img](http://box.kancloud.cn/2015-09-21_55ff98a8c659b.png)

上图中，很明显的标出了：自定义函数Foo.**proto**指向Function.prototype，Object.**proto**指向Function.prototype，唉，怎么还有一个……Function.**proto**指向Function.prototype？这不成了循环引用了？

对！是一个环形结构。

其实稍微想一下就明白了。Function也是一个函数，函数是一种对象，也有**proto**属性。既然是函数，那么它一定是被Function创建。所以——Function是被自身创建的。所以它的**proto**指向了自身的Prototype。

篇幅不少了，估计也都看烦了。快结束了。

最后一个问题：Function.prototype指向的对象，它的**proto**是不是也指向Object.prototype？

答案是肯定的。因为Function.prototype指向的对象也是一个普通的被Object创建的对象，所以也遵循基本的规则。

![img](http://box.kancloud.cn/2015-09-21_55ff98a8e08ed.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff98e57b560.png)

上图中，f1这个对象是被Foo创建，但是“f1 instanceof Object”为什么是true呢？

至于为什么过会儿再说，先把instanceof判断的规则告诉大家。根据以上代码看下图：

![img](http://box.kancloud.cn/2015-09-21_55ff98eb1ac7c.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff98eb39d8e.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff98eb49706.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99764b28b.png)

以上代码中，f1是Foo函数new出来的对象，f1.a是f1对象的基本属性，f1.b是怎么来的呢？——从Foo.prototype得来，因为f1.**proto**指向的是Foo.prototype

访问一个对象的属性时，先在基本属性中查找，如果没有，再沿着**proto**这条链向上找，这就是原型链。

看图说话：

![img](http://box.kancloud.cn/2015-09-21_55ff99769eadf.png)

上图中，访问f1.b时，f1的基本属性中没有b，于是沿着**proto**找到了Foo.prototype.b。

那么我们在实际应用中如何区分一个属性到底是基本的还是从原型中找到的呢？大家可能都知道答案了——hasOwnProperty，特别是在for…in…循环中，一定要注意。

![img](http://box.kancloud.cn/2015-09-21_55ff997bba534.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff997bd4463.png)

对象的原型链是沿着**proto**这条线走的，因此在查找f1.hasOwnProperty属性时，就会顺着原型链一直查找到Object.prototype。
...
![img](http://box.kancloud.cn/2015-09-21_55ff997c048fa.png)

看到了吧，有call、length等这些属性。

那怎么还有hasOwnProperty呢？——那是Function.prototype继承自Object.prototype的方法。有疑问可以看看上一节将instanceof时候那个大图，看看Function.prototype.**proto**是否指向Object.prototype。
...
![img](http://box.kancloud.cn/2015-09-21_55ff999143096.png)

其次，如果继承的方法不合适，可以做出修改。

![img](http://box.kancloud.cn/2015-09-21_55ff999167064.png)

如上图，Object和Array的toString()方法不一样。肯定是Array.prototype.toString()方法做了修改。

同理，我也可以自定义一个函数，并自己去修改prototype.toString()方法。

![img](http://box.kancloud.cn/2015-09-21_55ff999177145.png)

最后，如果感觉当前缺少你要用的方法，可以自己去创建。

例如在json2.js源码中，为Date、String、Number、Boolean方法添加一个toJSON的属性。

![img](http://box.kancloud.cn/2015-09-21_55ff999186546.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99afe6483.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99b011456.png)

这是第一种情况。

下面还有。先来个简单的。

![img](http://box.kancloud.cn/2015-09-21_55ff99b02c4c8.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99b045b13.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99b05a8d6.png)

其次，eval代码接收的也是一段文本形式的代码。

![img](http://box.kancloud.cn/2015-09-21_55ff99b06efa0.png)

最后，函数体是代码段是因为函数在创建时，本质上是 new Function(…) 得来的，其中需要传入一个文本形式的参数作为函数体。

 ![img](http://box.kancloud.cn/2015-09-21_55ff99b08ec0e.png)
...
继续上一篇文章（[http://www.cnblogs.com/wangfupeng1988/p/3986420.html）的内容。](http://www.cnblogs.com/wangfupeng1988/p/3986420.html%EF%BC%89%E7%9A%84%E5%86%85%E5%AE%B9%E3%80%82)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99c6460ef.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff99c655833.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a462085e.png)

以上代码中，fn不仅作为一个对象的一个属性，而且的确是作为对象的一个属性被调用。结果this就是obj对象。

注意，如果fn函数不作为obj的一个属性被调用，会是什么结果呢？

![img](http://box.kancloud.cn/2015-09-21_55ff9a462fbb7.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a4649de4.png)

**情况4\****：全局 &** **调用普通函数**

在全局环境下，this永远是window，这个应该没有非议。

![img](http://box.kancloud.cn/2015-09-21_55ff9a4b603f0.png)

普通函数在调用时，其中的this也都是window。

![img](http://box.kancloud.cn/2015-09-21_55ff9a4b6fcef.png)

以上代码很好理解。

不过下面的情况你需要注意一下：

![img](http://box.kancloud.cn/2015-09-21_55ff9a4b84ea6.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a4b99c1a.png)
...
![img](http://images.cnitblog.com/blog/138012/201409/270954235926791.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a68240a7.png)

可根据以下代码来详细介绍上下文栈的压栈、出栈过程。

![img](http://box.kancloud.cn/2015-09-21_55ff9a6db72ef.png)

如上代码。

在执行代码之前，首先将创建全局上下文环境。

![img](http://box.kancloud.cn/2015-09-21_55ff9a6dcc4d5.png)

然后是代码执行。代码执行到第12行之前，上下文环境中的变量都在执行过程中被赋值。

![img](http://box.kancloud.cn/2015-09-21_55ff9a6ddbe46.png)

执行到第13行，调用bar函数。

跳转到bar函数内部，执行函数体语句之前，会创建一个新的执行上下文环境。

![img](http://box.kancloud.cn/2015-09-21_55ff9a7300fc4.png)

并将这个执行上下文环境压栈，设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9a786d3c1.png)

执行到第5行，又调用了fn函数。进入fn函数，在执行函数体语句之前，会创建fn函数的执行上下文环境，并压栈，设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9a7881927.png)

待第5行执行完毕，即fn函数执行完毕后，此次调用fn所生成的上下文环境出栈，并且被销毁（已经用完了，就要及时销毁，释放内存）。

![img](http://box.kancloud.cn/2015-09-21_55ff9a789672f.png)

同理，待第13行执行完毕，即bar函数执行完毕后，调用bar函数所生成的上下文环境出栈，并且被销毁（已经用完了，就要及时销毁，释放内存）。

![img](http://box.kancloud.cn/2015-09-21_55ff9a78ab0b7.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a909e928.png)

再比如for语句：

![img](http://box.kancloud.cn/2015-09-21_55ff9a90b39b8.png)

所以，我们在编写代码的时候，不要在“块”里面声明变量，要在代码的一开始就声明好了。以避免发生歧义。如：

![img](http://box.kancloud.cn/2015-09-21_55ff9a9112dae.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a912270b.png)

下面继续说作用域。作用域是一个很抽象的概念，类似于一个“地盘”

![img](http://box.kancloud.cn/2015-09-21_55ff9a913c41f.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9a9156556.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9abfb586f.png)
...
[http://www.cnblogs.com/wangfupeng1988/p/3986420.html](http://www.cnblogs.com/wangfupeng1988/p/3986420.html)

[http://www.cnblogs.com/wangfupeng1988/p/3987563.html](http://www.cnblogs.com/wangfupeng1988/p/3987563.html)

第一步，在加载程序时，已经确定了全局上下文环境，并随着程序的执行而对变量就行赋值。

![img](http://box.kancloud.cn/2015-09-21_55ff9abfef02a.png)

第二步，程序执行到第27行，调用fn(10)，此时生成此次调用fn函数时的上下文环境，压栈，并将此上下文环境设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9ac01fea0.png)

第三步，执行到第23行时，调用bar(100)，生成此次调用的上下文环境，压栈，并设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9ac048ceb.png)

第四步，执行完第23行，bar(100)调用完成。则bar(100)上下文环境被销毁。接着执行第24行，调用bar(200)，则又生成bar(200)的上下文环境，压栈，设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9ac0bb5cd.png)

第五步，执行完第24行，则bar(200)调用结束，其上下文环境被销毁。此时会回到fn(10)上下文环境，变为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9acad9ad6.png)

第六步，执行完第27行代码，fn(10)执行完成之后，fn(10)上下文环境被销毁，全局上下文环境又回到活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9acb01870.png)

结束了。像老太太的裹脚布——又臭又长！

最后我们可以把以上这几个图片连接起来看看。

![img](http://box.kancloud.cn/2015-09-21_55ff9acb19654.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9aa9c22ca.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9adef1f47.png)

如上代码，bar函数作为返回值，赋值给f1变量。执行f1(15)时，用到了fn作用域下的max变量的值。至于如何跨作用域取值，可以参考上一节。

**第二，函数作为参数被传递**

![img](http://box.kancloud.cn/2015-09-21_55ff9adf50c84.png)
...
在前面讲执行上下文栈时（[http://www.cnblogs.com/wangfupeng1988/p/3989357.html），我们提到当一个函数被调用完成之后，其执行上下文环境将被销毁，其中的变量也会被同时销毁。](http://www.cnblogs.com/wangfupeng1988/p/3989357.html%EF%BC%89%EF%BC%8C%E6%88%91%E4%BB%AC%E6%8F%90%E5%88%B0%E5%BD%93%E4%B8%80%E4%B8%AA%E5%87%BD%E6%95%B0%E8%A2%AB%E8%B0%83%E7%94%A8%E5%AE%8C%E6%88%90%E4%B9%8B%E5%90%8E%EF%BC%8C%E5%85%B6%E6%89%A7%E8%A1%8C%E4%B8%8A%E4%B8%8B%E6%96%87%E7%8E%AF%E5%A2%83%E5%B0%86%E8%A2%AB%E9%94%80%E6%AF%81%EF%BC%8C%E5%85%B6%E4%B8%AD%E7%9A%84%E5%8F%98%E9%87%8F%E4%B9%9F%E4%BC%9A%E8%A2%AB%E5%90%8C%E6%97%B6%E9%94%80%E6%AF%81%E3%80%82)

但是在当时那篇文章中留了一个问号——有些情况下，函数调用完成之后，其执行上下文环境不会接着被销毁。这就是需要理解闭包的核心内容。

咱们可以拿本文的第一段代码（稍作修改）来分析一下。

![img](http://box.kancloud.cn/2015-09-21_55ff9adf6ac9c.png)

第一步，代码执行前生成全局上下文环境，并在执行时对其中的变量进行赋值。此时全局上下文环境是活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9adf84cdb.png)

第二步，执行第17行代码时，调用fn()，产生fn()执行上下文环境，压栈，并设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9adf9a25c.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9adfaff9a.png)

第四步，执行到第20行，执行f1(15)，即执行bar(15)，创建bar(15)上下文环境，并将其设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9adfc3bb3.png)
...
![img](http://box.kancloud.cn/2015-09-21_55ff9b2125ff5.png)

第二，程序执行之前，会生成全局上下文环境，并在程序执行时，对其中的变量赋值。

![img](http://box.kancloud.cn/2015-09-21_55ff9b213a759.png)

第三，程序执行到第17行，调用fn(5)，会产生fn(5)的上下文环境，并压栈，并设置为活动状态。

![img](http://box.kancloud.cn/2015-09-21_55ff9b214fa83.png)

第四，执行完第17行，fn(5)的返回值赋值给了f1。此时执行上下文环境又重新回到全局，但是fn(5)的上下文环境不能就此销毁，因为其中有闭包的引用（可翻看前面文章，此处不再赘述）。

![img](http://box.kancloud.cn/2015-09-21_55ff9b2164ead.png)

第五，继续执行第18行，再次调用fn函数——fn(10)。产生fn(5)的上下文环境，并压栈，并设置为活动状态。但是此时fn(5)的上下文环境还在内存中——一个作用域下同时存在两个上下文环境。

![img](http://box.kancloud.cn/2015-09-21_55ff9b217e658.png)
